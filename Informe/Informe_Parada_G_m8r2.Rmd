---
title: "Informe M8R2"
subtitle: "Esperanza de Vida y GDP PPP en Suramérica (1875-2025)"
...
# Min, max y diferencia por país Esperanza de Vida y GDP per cápita PPP (1875-2025)
...
- Aunque todos han seguido una tendencia al aumento de la Esperanza de Vida a la vez que aumenta su GDP per cápita PPP, se observa que esto no es una relación perfecta, por ejemplo `r resumen_pais$Países[which.max(resumen_pais$max.Esp.Vida)]` es el país que alcanza la mayor Esperanza de Vida con `r max(resumen_pais$max.Esp.Vida)` años, teniendo uno de los GDP per cápita PPP más bajos del grupo. 

<br><br>


```{r, echo=FALSE, fig.align='center', fig.width = 7, fig.height = 6}
# Histograma Esperanza de vida en Suramérica (1875 - 2025): 

# Creación de data frame para los datos de Suramérica
mediaSur <- data.frame(Países=rep("Suramérica",151),
                        Año=seq(1875, 2025))

# Calculo de media de Esperanza de Vida y GDP per cáoita PPP para Suramérica
df_m <- df_tablaI %>%
     group_by(Año) %>%
...
# Gráfico histograma Esperanza de vida en Suramérica (1875 - 2025)
...
- Luego incrementa rápidamente y se estabiliza un poco entre el rango de 67 a 80 años.
...
#Plot de Esperanza de vida por país
...
- Bolivia y Guyana se han quedado rezagados en el aumento de la Esperanza de Vida. Guyana es un caso interesante porque en los últimos años ha tenido un crecimiento económico acelerado, pero esto no se ha reflejado por los momentos en una mejora de la Esperanza de Vida.

<br><br>


```{r, echo=FALSE, fig.align='center', fig.width = 7, fig.height = 6}
# Plot GDP per cápita PPP por país (1875–2025)
 ggplot(df_tablaI, aes(x = Año, y = GDP, color = Países)) +
    geom_line(size = 1.2) +
    labs(
        title = "GDP per cápita PPP por país (1875–2025)",
        x = "Año",
        y = "GDP per cápita PPP ($)",
        color = "Países"
    ) +
    scale_x_continuous(breaks = seq(0, max(df_tablaI$Año), by = 25)) 
...
# Gráfico de líneas Media GDP per capita PPP Suramérica (1875-2025)

g1 <- ggplot(mediaSur, aes(x = Año, y = GDP, color = Países)) +
  geom_line(size = 1.7) +
  labs(
    title = "Media GDP per capita PPP Suramérica (1875-2025)",
    x = "Año",
    y = "GDP per cápita PPP ($)",
    color = "Países"
  ) +
...
# Gráfico de líneas Media Esperanza de Vida Suramérica (1875–2025)
...
# Grid para que aparezcan los dos gráficos uno al lado del otro
...
- El GDP per cápita PPP muestra una tendencia parecida. Desde 1930 aumenta hasta llegar a una media de `r format(mean(resumen_pais$max.GDP), scientific = FALSE)`\$.


<br><br>

# Correlación entre GDP per cápita PPP y Esperanza de Vida Suramérica (1875-2025)
...
# Plot de la correlación entre GDP per cápita PPP y Esperanza de Vida
ggplot(df_presente, aes(x = GDP, y = life_expectancy)) + 
  geom_point(col="coral2") +
  geom_smooth(method = "lm", se = FALSE) +
  ylim(0, 100) +
    scale_x_continuous(
  breaks = c(10000, 20000, 30000, 40000, 50000, 60000, 70000), 
  labels = c("10k", "20k", "30k", "40k", "50k", "60k", "70k") 
) +
  labs(x = "GDP per capita PPP ($)", y = "Esperanza de Vida (años)", 
       title = "Correlación entre GDP per cápita PPP y Esperanza de Vida") 
...
- El coeficiente de correlación de Pearson de ambas variables para Suramérica es de: `r cor_Infor <- cor.test(df_presente$GDP, df_presente$life_expectancy, method = "pearson"); round(cor_Infor$estimate,2)`.
